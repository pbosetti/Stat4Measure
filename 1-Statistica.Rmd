...
Le distribuzioni discrete hanno valore solo sui numeri interi. Le più comuni sono `geom` (geometrica), `binom` (binomiale), `pois` (Poisson). I grafici vengono generalmente riportati con linee verticali, usando l'opzione `typ="h"` nei comandi di plot (Fig. \ref{fig:dgeom})

```{r dgeom, fig.margin=T, fig.cap="PDF distribuzione geometrica"}
x <- 1:20
...
Inoltre, ricordarsi che la CDF della variabile casuale $X$ può riportare la coda alta (_upper tail_):
$$
F_{X,U}(x) = \mathrm{P}(X \leq x) = \sum_{x_i\leq x}p(x_i)
$$
e la coda bassa (_lower tail_):
$$
F_{X,L}(x) = \mathrm{P}(X > x) = \sum_{x_i > x}p(x_i)
$$
Per default, R considera la *lower tail* (`lower.tail=TRUE`, Fig. \ref{fig:pgeom}):

```{r pgeom, fig.margin=T, fig.cap="CDF distribuzione geometrica"}
...
```{r dnorm, fig.margin=T, fig.cap="PDF distribuzione normale"}
...
Per realizzare il grafico della funzione quantile è necessario ricordarsi che essa è l'inversa della CDF e, quindi, è definita solo nell'intervallo $(0,1)$ e va all'infinito agli estremi (Fig. \ref{fig:qnorm}):

```{r qnorm, fig.margin=T, fig.cap="Quantile distribuzione normale"}
...
Possiamo studiare la *convergenza in distribuzione* (Fig. \ref{fig:convdist}):

```{r convdist, fig.margin=T, fig.cap="Convergenza della media"}
set.seed(123)
n <- 1:1000
...
Vediamo ora come utilizzare i data frame per realizzare strutture dati più complesse (Fig. \ref{fig:norm-t}):

```{r norm-t, fig.margin=T, fig.cap="Confronto T di Student con normale standard"}
...
```{r t-points, warning=F}
file <- "t_values.txt"
n <- 1:120
p <- c(0.4, 0.25, 0.1, 0.05, 0.025, 
...
knitr::kable(head(m), 
             caption = "Probabilità della distribuzione T di Student")
```
...
Si noti che `write.csv()` usa la virgola come separatore di campo e il punto come separatore dei decimali, mentre `write.csv2()` usa il punto e virgola come separatore di campo e la virgola come separatore dei decimali. Quindi, `write.csv2()` è da usarsi se si intende importare il file creato, ad esempio, in versioni di Excel localizzate in Italiano o in lingue che usano la  come separatore decimale.
...
Si noti che la funzione `mode()` già esiste e ritorna lo *storage mode* di un oggetto. Inoltre, si noti che `mymode()` restituisce *il primo* elemento più frequente, tralasciando eventuali pari merito. In genere, è opportuno ordinare il vettore in modo da restituire il più comune e più grande (o più piccolo) elemento:
...
```{r density, fig.margin=T, fig.cap="Campione di dati casuali"}
set.seed(123)
n <- 100
v <- rnorm(n, 12, 1.5)
plot(v)
...
```
La serie in Fig \ref{fig:density} non mostra tendenze o pattern (ovviamente!) e studiando i quantili osserviamo che la distribuzione appare leggermente gobba a sinistra, dato che la mediana è leggermente più bassa della media.
...
La versione continua dell'istogramma è ottenuta con la funzione `density()`, che è utile confrontare con la distribuzione di riferimento (in questo caso la normale, Fig. \ref{fig:hist}):

```{r hist, fig.margin=T, fig.cap="Istogramma"}
hist(v, freq=F) # freq=T per conteggi invece di frequenze
lines(density(v))
...
Nel caso di campioni bi-variati o multi-variati uno strumento molto utile per l'osservazione preliminare dei dati è il boxplot (Fig. \ref{fig:boxplot}). Carichiamo i dati che riportano il tempo di reazione di un processo chimico in funzione di due diversi *trattamenti*, cioè condizioni di processo:

```{r boxplot, fig.margin=T, fig.cap="Boxplot"}
...
A volte è utile realizzare un *diagramma di Pareto* per visualizzare la distribuzione cumulativa. La libreria `qcc` mette a disposizione il la funzione `pareto.chart()`:

```{r pareto, fig.margin=F, fig.dim=c(7,4), fig.cap="Diagramma di Pareto"}
library(qcc)
set.seed(123)
n <- 10
...
È evidente che la potenza di un test dipende dalla dimensione del campione: più piccolo è il campione, più bassa sarà la potenza a pari condizioni.
...
```{r, OCC, fig.margin=T, fig.cap="Curva caratteristica operativa per un T-test"}
d <- seq(0, 10, 0.05)
nv <- c(3:10, 20, 30, 50)
...
df_prep <- data.frame(
  ...
  Strength=NA)
# riordinata secondo la sequenza  casuale
df_prep <- df_prep[order(df_prep$Run),]
str(df_prep)
```
...
```{r, fig.margin=T, fig.cap="Boxplot dell'esperimento sui filati di cotone"}
...
```{r, fig.margin=T, fig.cap="Plot di Tukey"}
...
È naturale estendere l'analisi della varianza a casi multi-variati, in cui abbiamo due o più fattori, ciascuno con due o più trattamenti. Nel caso a due fattori con $a$ e $b$ trattamenti (o livelli) il modello statistico è:
$$
...
```{r, fig.margin=T, fig.cap="Diagramma di interazione"}
...
```{r, fig.margin=T, fig.cap="Grafico Quantile-Quantile"}
library(car)
...
```{r, fig.margin=T, fig.cap="Grafico dei residui"}
par(mfrow=c(1,2))
...
```{r, warning=FALSE, fig.margin=T, fig.cap="Diagramma di Daniel degli effetti"}
library(FrF2)
...
```{r, warning=FALSE, fig.margin=T, fig.cap="Diagramma Quantile-Quantile"}
invisible(qqPlot(df.lm2))
```
```{r, warning=FALSE, fig.margin=T, fig.cap="Diagramma dei residui"}
...
```{r, fig.margin=T, fig.cap="Diagramma Box-Cox"}
library(MASS)
...
```{r, fig.margin=T, fig.cap="Diagramma dei residui, inverso"}
...
```{r, fig.margin=T, fig.cap="Diagramma dei residui, logaritmo"}
...
```{r, fig.margin=T, fig.cap="Grafico di Daniel degli effetti"}
...
```{r, fig.margin=T, fig.cap="Diagramma dei residui"}
...
```{r, fig.show='hold', fig.fullwidth=T, fig.dim=c(5, 6), fig.cap="Grafici di interazione"}
attach(df)
#par(mfrow = c(1, 3))
