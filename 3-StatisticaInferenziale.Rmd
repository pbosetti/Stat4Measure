...
Ovviamente R dispone di funzioni per calcolare automaticamente la tabella ANOVA. Tornando all'esempio dei filati in cotone, possiamo calcolare la tabella ANOVA in questo modo. Per prima cosa è necessario formulare il **modello lineare**:
```{r}
df.lm <- lm(Strength~Cotton, data=df)
```
La notazione `Strength~Cotton` corrisponde al modello delle medie $y_{ij}=\mu_i+\varepsilon_{ij}$, avendo rimosso i residui e sostituito l'uguale con una tilde.
