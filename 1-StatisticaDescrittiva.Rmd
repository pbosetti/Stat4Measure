...
Se $x_1, x_2, \dots,x_n$ sono $n$ variabili casuali indipendenti e identicamente distribuite (IID) con $E(x_i)=\mu$ e $V(x_i)=\sigma^2~~\forall i\in\{1,2,\dots,n\}$ (entrambi finiti), e $y=x_1+x_2+\dots+x_n$, allora:
\[
...
$z_i\sim\mathcal(N)(0,1)~~\forall i\in\{1, 2, \dots,k\}$. Allora risulta
...
\[
\frac{(y_i-\bar y)^2}{\sigma^2}\sim \mathcal{N}(0,1)~~\forall i\in\{1,2,\dots,k\}
\]
e quindi:
\[
...
\end{equation}
è una cariabile casuale distribuita come una F di Snedecor, con funzione di distribuzione (vedi Fig. \ref{fig:snedecor}):
\begin{equation}
...
```{marginfigure}
Si noti che in inglese la moda è *mode*; tuttavia la funzione `mode()` è già deinita in R e rappresenta lo *storage mode* di una variabile. Per questo motivo usiamo qui il nome italiano.
...
Ora confrontiamo i parametri media e deviazione standad con le stime su un grafico a dispersione:
```{r}
m_s = mean(df$x)
s_s = sd(df$x)
