...
Applicando un carico all'estremità libera della mensola essa si flette, mettendo in trazione la superficie superiore e in compressione quella inferiore. L'estensimetro misura questa trazione, la cui variazione è proporzionale alla variaazione di forza applicata all'estremo libero.
...
curve(model(x, 20, parameters), from=0, to=100, ylab="V (Volt)", xlab="F (N)")
curve(temp(x), from=0, to=20, xlab="Tempo (h)", ylab="Temperatura (°C)")
```
...
Per la lunghezza (notare l'assimmetria):
```{r fig.margin=T, tidy=F}
