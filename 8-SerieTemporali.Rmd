...
```{marginfigure}
**Nota**: $\nabla^2x_t=(1-B)^2x_t$ può essere espresso come `diff(x, diff=2)` ma **non** come `diff(x, 2)` che corrisponde invece a $x_t - x_{t-2}$, cioè $(1-B^2)x_t$.
```
...
# Modelli statistici per le delle serie temporali
...
Prendendo in considerazione le **serie temporali stazionarie**, i due modelli più naturali per la loro descrizione sono il modello **autoregressivo**, o AR, e il modello **a media mobile**, o MA (da *moving average*), e la loro combinazione ARMA.

`r newthought("Modello Auto-Regressivo (AR)")`

Un modello AR è basato sull'idea che una determinata osservazione $x_t$ al tempo $t$ possa essere espressa come combinazione lineare di $p$ valori precedenti $x_{t-1}, x_{t-2},\dots,x_{t-p}$:
\begin{dfn}[Modello auto-regressivo]
Un modello AR di ordine $p$, abbreviato in $\mathrm{AR}(p)$, ha la forma
\begin{equation}
x_t=\phi_1 x_{t-1} + \phi_2 x_{t-2}+\dots+\phi_p x_{t-p} + w_t \label{eq:AR}
\end{equation}
dove $x_t$ è stazionaria in senso ampio, $w_t\sim\mathcal{N}(0, \sigma^2_w)$\footnote{Le $w_t$ sono anche chiamate \strong{innovazioni}, dato che sono l'unico contributo \emph{nuovo} di un punto rispetto ai precedenti.} e $\phi_1, \phi_2,\dots,\phi_p$, con $\phi_p\neq0$ sono costanti. La media di $x_t$ in (\ref{eq:AR}) è nulla; se non può essere nulla, si sostituisce $x_t$ con $x_t - \mu$ per ottenere:
\begin{equation}
...
Ricordando la definizione dell'operatore *backshift*, la (\ref{eq:AR}) può essere scritta come:
\begin{equation}
(1-\phi_1 B - \phi_2 B^2 -\dots-\phi_p B^p)x_t=w_t
\end{equation}
o ancora più concisamente come:
\begin{equation}
\Phi_p(B)x_t=w_t\label{eq:AR2}
\end{equation}
dove $\Phi_p(B)=1-\phi_1B-\phi_2B^2-\dots-\phi_pB^p$ è detto **operatore autoregressivo**.

Effettuare la regressione di un modello $\mathrm{AR}(p)$ su una serie storica $x_t$ significa quindi adattare il modello $\Phi^p(B)\hat x_t=w_t$ identificando i coefficienti di $\Phi^p(\cdot)$ che minimizzano i residui quadratici medi, essendo i residui $\varepsilon_t = x_t - \hat x_t = x_t - \Phi_p^{-1}(B)w_t$ (Ammesso che esista l'inversa $\Phi_p^{-1}(\cdot)$).

`r newthought("Modello a media mobile (MA)")`

Alternativamente, è possibile immaginare il caso in cui la generica osservazione $x_t$ è espressa come combinazione lineare **del disturbo** agente sulle $q$ osservazioni precedenti:
\begin{dfn}[Modello a media mobile]
Un modello MA di ordine $q$, abbreviato come $\mathrm{MA}(q)$ è definito come
\begin{equation}
x_t = w_t + \theta_1 w_{t-1} + \theta_2 w_{t-2} + \dots + \theta_q w_{t-q} \label{eq:MA}
\end{equation}
dove $w_t\sim\mathcal{N}(0, \sigma_w^2)$ e $\theta_1, \theta_2,\dots,\theta_q,~\theta_q\neq0$ sono parametri costanti.
\end{dfn}

Analogamente al caso $\mathrm{AR}(p)$, è possibile definire l'**operatore media mobile** di ordine $q$, $\Theta_q(B)=(1+\theta_1 B+\theta_2 B^2+\dots+\theta_q B^q)$ tale per cui la (\ref{eq:MA}) può essere scritta come:
\begin{equation}
x_t=\Theta_q(B)w_t \label{eq:MA2}
\end{equation}

Come sopra, la regressione di un modello $\mathrm{MA}(q)$ ad una serie storica $x_t$ significa identificare i termini di $\Theta_q(\cdot)$ che minimizzano i residui quadratici medi, definiti come $\varepsilon_t = x_t - \hat x_t = x_t - \Theta_q(B)$.

`r newthought("Modello autoregressivo a media mobile (ARMA)")`

L'ovvia estensione risulta dalla combinazione delle (\ref{eq:AR}) e (\ref{eq:MA}):
\begin{dfn}[Modello autoregressivo a media mobile]
...
+w_t + \theta_1 w_{t-1} + \theta_2 w_{t-2} + \dots + \theta_q w_{t-q}
\end{multline}
...
`r newthought("Modello autoregressivo, integrato, a media mobile (ARIMA)")`

I processi $\mathrm{ARMA}(p,q)$ sono adatti a descrivere serie **stazionarie** e, quindi, non possono descrivere serie temporali come quelli di Fig. \ref{fig:chicken}.

Nel primo capitolo di questa Sezione abbiamo visto come l'operatore differenza $\nabla$ abbia un effetto stabilizzante sulla serie storica $x_t$. Viene quindi spontaneo utilizzarlo per stabilizzare la serie prima di modellarla come un processo $\mathrm{ARMA}(p,q)$, ottenendo la seguente:
\begin{dfn}[Modello autoregressivo, integrato, a media mobile]
Un processo $x_t$ è detto $\mathrm{ARIMA}(p,d,q)$ quando:
\begin{equation}
\nabla^d x_t = (1-B)^d x_t
\end{equation}
è un processo $\mathrm{ARMA}(p,q)$. In generale, il processo $\mathrm{ARIMA}(p,d,q)$ può essere scritto come:
\begin{equation}
\Phi_p(B)(1.B)^dx_t=\Theta_q(B)w_t
\end{equation}
e se $E(\nabla^dx_t)=\mu$, allora:
\begin{equation}
\Phi_p(B)(1-B)^dx_t=\delta + \Theta_q(B)w_t
\end{equation}
dove $\delta=\mu(1-\phi_1-\dots-\phi_p)$.
\end{dfn}

## Stabilità e unicità dei modelli ARMA

Consideriamo un modello $\mathrm{AR}(1)$: possiamo sviluppare la formula ricorsiva come:
\begin{eqnarray*}
x_t &=& \phi x_{t-1}+w_t = \phi(\phi x_{t-2}+w_t) + w_t \\
 &\vdots& \\
...
\end{equation}
Un modello AR non stabile è detto anche **anticausale**, perché si può dimostrare che per essere stabilizzato richiede la conoscenza delle osservazioni future.

Per il caso $\mathrm{AR}(2)$, possiamo ricavare il dominio di stabilità sul piano $p_1, p_2$ in questo modo:
```{r fig.margin=T}
chk <- function(x, y) min(Mod(polyroot(c(1, -x, -y))))
rng <- seq(-2.5, 2.5, 0.01)
expand.grid(p1=rng, p2=rng) %>%
  tibble() %>%
  filter(p1*p2!=0) %>%
  rowwise() %>%
  mutate(root=chk(p1,p2)) %>%
  ggplot(aes(x=p1, y=p2, z=root)) +
  geom_contour_filled(breaks=c(0, 1, Inf)) +
  scale_color_steps()
```

Se invece consideriamo un modello $\mathrm{MA}(1)$ abbiamo un potenziale problema di carattere differente. Consideriamo i due modelli:
\begin{eqnarray}
x_t &=& w_t + k w_{t-1}, ~w_t\sim\mathcal{N}(0, 1) \\
y_t &=& \nu_t + 1/k \nu_{t-1}, ~w_t\sim\mathcal{N}(0, k^2) 
\end{eqnarray}
È evidente che $x_t$ e $\nu_t$ hanno la stessa ACF e sono indistinguibili, dato che noi non conosciamo le innovazioni $w_t$ e $\nu_t$ ma solo le due serie. Si può dimostrare che il risultato è generale (cioè non dipende dell'orine $q$). È quindi necessario scegliere una delle due forme alternative. Per individuare quale, riscriviamo la serie come $w_t=-\theta x_{t-1}+x_t$, che ha la stessa forma della $\mathrm{AR}(1)$ (\ref{eq:AR}). Possiamo quindi scegliere l'alternativa che rispetta lo stesso criterio di *stabilità* (\ref{eq:ARstab}):
\begin{equation}
\left|\sum_{i=1}^q \theta_i\right| < 1 \label{eq:MAstab}
\end{equation}
Tale alternativa si dice **invertibile**.

```{marginfigure}
**Nota**: Il termine **invertibile** deriva dal fatto che è possibile trasformare un modello ARMA in un modello MA infinito, cioè con un infinito numero di permini convergenti a zero, e per quest'ultimo vale la proprietà di invertibilità. 
```

\boxedpar{
In generale, si può dimostrare che i criteri in (\ref{eq:ARstab}) e (\ref{eq:MAstab}) corrispondono a imporre il requisito che le radici complesse dei polinomi $\Phi_p(z)=1-\phi_1 z-\phi_2 z^2 - \dots -\phi_p z^p$ e $\Theta_q(z)=1+\theta_1 z+\theta_2 z^2 + \dots +\theta_q z^q$ (con $\phi_p$ e $\theta_q$ non nulli) siano \strong{tutte} strettamente fuori dal cerchio unitario sul piano complesso.
}
Per i dettagli delle dimostrazioni vedi @Shumway2017.
```{marginfigure}
**Nota**: È evidente che un $\mathrm{AR}(p), ~p>1$ ha **memoria infinita**: dato che ogni $x_t$ dipende da almeno un valore precedente e la formula è ricorsiva, la memoria (pur attenuandosi esponenzialmente) non è mai nulla.

Viceversa, è altrettanto evidente che un processo $\mathrm{MA}(q), ~q>1$ **non ha memoria** oltre $q$ elementi precedenti.
```


Ad esempio, il modello $\mathrm{ARMA}(2,3)$ con polinomi $\Phi_2(z)= 1 + 0.9z + 0.1z^2$ e $\theta_q(z) = 1 + 2z + 15z^2$ può essere verificato con R mediante la funzione `polyroot()`:
```{r}
abs(polyroot(c(1, -0.9, -0.1)))
abs(polyroot(c(1, 2, 15)))
abs(polyroot(c(1, 1/2, 1/15)))
```

Quindi: la parte AR del modello non è causale (perché solo una delle radici è maggiore di 1) e la parte MA non è invertibile. Quest'ultima può però essere sostituita dal polinomio con fattori inversi.

Se i criteri di causalità e invertibilità riguardano, rispettivamente, la parte AR e la parte MA di un modello **in maniera indipendente**, c'è un criterio che invece dipende dai valori **combinati** dei polinomi $\phi^p$ e $\theta^q$.

Consideriamo il processo $x_t = w_t$: si tratta ovviamente di puro rumore casuale. Se moltiplichiamo entrambi i lati per $1-0.5B$ otteniamo $(1-0.5B)x_t=(1-0.5B)w_t$, cioè
\[
x_t=0.5x_{t-1}-0.5w_{t-1}+w_t
\]
che *sembra* un modello $\mathrm{ARMA}(1,1)$, ma ovviamente è sempre lo stesso rumore casuale $x_t=w_t$.

Questo ci dice che un modello ARMA può avere parametri ridondanti e ci serve quindi un criterio per identificare questi casi.

Di fatto, è sufficiente scomporre in fattori i polinomi $\Phi_p$ e $\Theta_q$ ed eliminare i fattori comuni. È più semplice con un esempio. Consideriamo il processo
\[
x_t=0.4x_{t-1}+0.45x_{t-2}+w_t + w_{t-1}+0.25w_{t-2}
\]
che usando l'operatore $B$ diventa:
\[
...
```
Come si vede, si può scrivere $\Phi_p(B)=(1-0.9B)(1+0.5B)$ e $\Theta_q(B)=(1+0.5B)^2$. Eliminando il fattore comune $(1+0.5B)$ otteniamo il modello 
\[
...
set.seed(0)
ts <- arima.sim(
  list(order=c(0,0,2), ma=c(0.5, 0.5)),
  n=150)
plot(ts)
acf(ts)

plot(0:10, ARMAacf(ma=c(0.5,0.3), lag.max=10), typ="h")
```

\boxedpar{
Quindi ai criteri di \strong{causalità} e di \strong{invertibilità} si aggiunge il criterio di \strong{non ridondanza} dei parametri, che si verifica eliminando ogni fattore comune dalla scomposizione in fattori dei polinomi $\Phi_p(B)$ e $\Theta_q(B)$.
}

# Simulazione e analisi dei processi AR(I)MA
```{marginfigure}
**Nota**: per semplicità e rapidità utilizzeremo qui le funzioni grafiche native di R, anziché ricorrere a `ggplot`. Il motivo è che le funzioni nativo comprendono già numerosi strumenti di analisi di serie temporali che non sono direttamente disponibili in `ggplot`, anche se grafici analoghi possono essere facilmente ottenuti in `ggplot` ma con qualche riga di codice in più.

Di conseguenza, si consiglia di utilizzare le funzioni native di seguito illustrate per la fase di *analisi*, lasciando `ggplot` per la realizzazione dei grafici finali di *presentazione* (per i quali, come detto sopra, la funzione `plot()` è incapace di trattare le date in maniera completa.)
```
`r newthought("Dato un certo processo, o serie temporale")`, è anzitutto necessario disporre di strumenti che consentano di analizzarlo e determinare i parametri ARIMA $p, d$, e $q$ più adatti a rappresentarlo. Anzitutto, però, è utile poter **creare** serie temporali a partire da parametri ARIMA noti e da una determinata sorgente delle **innovazioni** $w_t$. In R, la funzione `arima.sim()` consente di generare serie temporali (oggetti `ts`) a partire dai parametri:
```{r fig.margin=T}
set.seed(1)
N <- 300
innovations = rnorm(N)
...
La serie `ma2` può essere analizzata con la funzione di autocorrelazione: come abbiamo visto sopra, i processi $\mathrm{MA}(q)$ non hanno memoria oltre il $q$-esimo *lag*, quindi ci aspettiamo una ACF che si smorza marcatamente dopo due picchi:
```{r fig.margin=T}
acf(ma2, lag.max = 50)
```
In particolare, ricordandosi che la ACF comincia dal *lag* 0 che ha sempre valore 1, in questo caso osserviamo i due picchi a *lag* 1 e 2 che sono relativamente elevati, dopodiché c'è una brusca diminuzione e inizia un comportamento oscillatorio smorzato. Possiamo quindi supporre (correttamente!) che il modello in `ar2` sia $\mathrm{MA}(2)$.

Per quanto riguarda `ar2`, invece, si è visto che i processi AR hanno memoria infinita, quindi ci aspettiamo una ACF che si smorza gradatamente terminando in oscillazioni armoniche smorzate. Di fatto, come si osserva dal prossimo grafico, **la ACF è di praticamente inutile nello studio del comportamento autoregressivo di serie temporali**, nel senso che consente di capire che un modello ha una memoria di lungo periodo ed ha quindi un probabile contenuto AR, ma non consente di capire l'ordine $p$ di tale contributo: 
```{r fig.margin=T}
acf(ar2, lag.max = 50)
```

Per l'analisi dei processi AR è quindi più utile la **funzione di autocorrelazione parziale**, o PACF (*Partial ACF*). La PACF è la funzione che restituisce la autocorrelazione tra $x_t$ e $x_{t-\ell}$ per ogni dato *lag* $\ell$ **eliminando però dalla correlazione i termini compresi tra $t$ e $t-\ell$**. Applicandola a `ar2` otteniamo:
```{r fig.margin=T}
pacf(ar2, lag.max = 50)
```
Si noti anzitutto che la PACF **inizia a *lag* 1** anziché 0 come la ACF. In questo caso osserviamo quindi due picchi a *lag* 1 e 2 (il primo positivo, il secondo negativo), dopodiché iniziano le solite oscillazioni armoniche smorzate. possiamo quindi stimare un processo di tipo $\mathrm{AR}(2)$.
