...
Come sopra, la regressione di un modello $\mathrm{MA}(q)$ ad una serie storica $x_t$ significa identificare i termini di $\Phi_q(\cdot)$ che minimizzano i residui quadratici medi, definiti come $\varepsilon_t = x_t - \hat x_t = x_t - \Theta_q(B)$.
...
set.seed(0)
ts <- arima.sim(
  list(order=c(2,0,0), ar=c(-0.9, 0.1)),
  n=100)
plot(ts)
acf(ts)
```
